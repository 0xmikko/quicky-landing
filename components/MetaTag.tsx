...
import { isDev, SERVER_ADDR } from "../config";
...
export function MetaTag({ data }: MetaTagProps): React.ReactElement {
  useEffect(() => {
    if (process.browser) {
      if (document !== undefined) {
        const domain = isDev ? ".quicky.so" : "localhost";
        Analytics.init(SERVER_ADDR, domain).then(async () => {
  ...
  const defaultMetaImg =
    "https://storage.googleapis.com/quicky-apps/landing/embed/quicky.jpg";
  ...
  return (
    <Head>
      ...
      <meta property="og:url" content={`https://quicky.so${data.url}`} />
